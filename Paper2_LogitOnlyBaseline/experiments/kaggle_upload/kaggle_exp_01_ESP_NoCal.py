#!/usr/bin/env python3
"""
Kaggle Script — exp_01_ESP_NoCal (Raw Entropy Slope, NO Calibration)
=====================================================================
Self-contained: paste entire file into a single Kaggle notebook cell.

Validates the raw ESP signal BEFORE 3-scale calibration.
Ablation: Raw slope → Scale 1 (token z-norm) → compare vs loss baseline.
...
# ── Extractor: ESP only, NO calibration ──────────────────────────────────────
class ESPExtractor:
    """Raw entropy slope — no Scale 2/3 calibration."""
    ...
    def extract(self, text):
        ...
        if seq < self.min_tok:
            return {k: np.nan for k in [
                "esp_slope","neg_esp_slope","z_esp_slope",
                "h_mean","h_std","h_drop","h_curvature",
                "neg_mean_loss","minkprob_20","seq_len","n_tokens"]}
        ...
        # Scale 1 only: token-level z-norm entropy slope
        ...
        return {
            "esp_slope":     esp_slope,
            "neg_esp_slope": -esp_slope,   # higher = steeper decline = more member
            "z_esp_slope":   z_esp,         # + Scale 1 only
            "h_mean":        h_mean,
            "h_std":         h_std,
            "h_drop":        h_drop,
            "h_curvature":   h_curv,
...
# ── Ablation: show contribution of Scale 1 ───────────────────────────────────
def run_ablation(df):
    print(f"\n{'─'*50}\n  ABLATION: Raw ESP vs Scale 1\n{'─'*50}")
    checks = [
        ("esp_slope",     "(a) Raw esp_slope (lower=member)"),
        ("neg_esp_slope", "(b) Negated slope  (higher=member)"),
        ("z_esp_slope",   "(c) + Scale 1 (token z-norm)"),
        ("h_drop",        "(d) Entropy drop (first vs second half)"),
        ("neg_mean_loss", "(e) Loss baseline"),
        ("minkprob_20",   "(f) Min-K% baseline"),
    ]
    for col, desc in checks:
        ...
        print(f"  {desc:45s}  AUC={max(auc,1-auc):.4f}")

# ── Run Pipeline (no calibration) ────────────────────────────────────────────
def run_benchmark(model, tok, df, tag, max_len=512):
    ...
    print(f"  ✓ {len(df)} rows in {time.time()-t0:.0f}s  [NO calibration]")

    score_cols = [c for c in fdf.columns if c not in ["seq_len","n_tokens"]]
    print(f"\n{'─'*50}\n  RESULTS: {tag}  (no calibration)\n{'─'*50}")
    res = eval_auc(df, score_cols)
    for _, r in res.iterrows():
        m = "★" if "esp" in r["score"] else " "
        print(f"  {m} {r['score']:25s}  AUC={r['auc']:.4f} ({r['pol']})")

    best = res.iloc[0]["score"] if len(res) > 0 else "neg_esp_slope"
    print(f"\n  Per-language ({best}):")
    ...
    df.to_parquet(f"{odir}/esp_nocal_{tag}_{ts}.parquet", index=False)
    with open(f"{odir}/esp_nocal_{tag}_{ts}.json", "w") as f:
        json.dump({"tag": tag, "results": res.to_dict("records")}, f, indent=2)
    print(f"  Saved → {odir}/esp_nocal_{tag}_{ts}.*")
    return df, res
...
if __name__ == "__main__":
    ...
    print("\n" + "█"*60 + "\n  exp_01_ESP_NoCal: Poisoned Chalice\n" + "█"*60)
