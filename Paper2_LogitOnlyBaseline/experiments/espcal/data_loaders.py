...
def load_wikimia(cfg: Config) -> Dict[str, pd.DataFrame]:
    ...
    for length in cfg.wikimia_lengths:
        try:
            ...
            rows = [{"text": r["input"], "is_member": int(r["label"]), "subset": f"len{length}"} for r in ds]
...
def load_bookmia(cfg: Config) -> pd.DataFrame:
    ...
    try:
        ...
        rows = [{"text": r["snippet"], "is_member": int(r["label"]), "subset": f"book_{r.get('book_id', 0)}"} for r in ds]
        df = pd.DataFrame(rows)
        mem = df["is_member"].sum()
