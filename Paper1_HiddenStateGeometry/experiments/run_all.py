#!/usr/bin/env python3
"""
MultiGeo-MIA — Full Multi-Model Benchmark Evaluation
=====================================================
...
Usage:
    ...
    python run_all.py --no-multi-model      # single model per benchmark (fast)
"""
...
# Install deps (Kaggle-compatible)
...
import pandas as pd
import torch
from multigeo import Config, MultiGeoExperiment


def main():
    parser = argparse.ArgumentParser(description="MultiGeo-MIA Full Evaluation")
    parser.add_argument("--benchmark", type=str, default="all",
                        choices=["all", "poisoned_chalice", "wikimia", "mimir", "bookmia"])
    parser.add_argument("--no-multi-model", action="store_true",
                        help="Use single model per benchmark (faster)")
