"""
EXPERIMENT 17: ICP-MIA-SP (Random Mask Probing)
Method: In-Context Probing with self-perturbation (random masking).
Goal: Estimate optimization gap via log-likelihood improvement under probes.
      - Member: small improvement -> score near 0
      - Non-member: large improvement -> more negative score
...
# ============================================================================
# ICP-MIA-SP Attack (Random Mask Probing)
# ============================================================================
...
class Experiment:
    def __init__(self, args):
        self.args = args
        self.output_dir = Path(args.output_dir) / f"EXP17_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def run(self):
        df = self.load_data()
        attacker = ICPMIAAttack(self.args, self.model, self.tokenizer)
...
if __name__ == "__main__":
    ...
    print(f"[EXP17] Model: {Args.model_name}")
    Experiment(Args).run()
